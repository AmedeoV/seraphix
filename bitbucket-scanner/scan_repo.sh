...
# Scans individual Bitbucket repositories for leaked secrets
#
...
Usage:
    $0 <workspace/repo> [options]

Arguments:
    workspace/repo    Bitbucket repository in format "workspace/repo"
...
Examples:
    $0 atlassian/python-bitbucket
    $0 myworkspace/myrepo --commit abc123def
    $0 myworkspace/privaterepo --username myuser --app-password xxx
    $0 myworkspace/repo --telegram-chat-id 123456789 --debug
...
clone_repository() {
    local repo_name="$1"
    local repo_path="$2"
    local repo_url="https://bitbucket.org/$repo_name"
    ...
    if [ -n "$BITBUCKET_USERNAME" ] && [ -n "$BITBUCKET_APP_PASSWORD" ]; then
        repo_url="https://${BITBUCKET_USERNAME}:${BITBUCKET_APP_PASSWORD}@bitbucket.org/$repo_name"
    fi
    
    log_progress "Cloning repository: https://bitbucket.org/$repo_name"
    
    if ! git clone "$repo_url" "$repo_path" 2>"$TEMP_DIR/clone_error.log"; then
        log_error "Failed to clone repository: https://bitbucket.org/$repo_name"
        ...
        while IFS= read -r line; do
            if [ -n "$line" ] && echo "$line" | jq . >/dev/null 2>&1; then
                ...
                if [ "$is_verified" = true ]; then
                    ...
                    local metadata="{\"scan_timestamp\": \"$timestamp\", \"repository_name\": \"$repo_name\", \"source\": \"bitbucket\"}"
...
print_summary() {
    ...
    echo "📊 SCAN SUMMARY"
    echo "============================================================"
    echo "Repository: $repo_name (Bitbucket)"
...
parse_arguments() {
    ...
    if [ -z "$OUTPUT_FILE" ]; then
        ...
        # Create timestamped results directory inside bitbucket-scanner folder
        local results_dir="$SCRIPT_DIR/leaked_secrets_results/${timestamp}"
        mkdir -p "$results_dir"
        
        OUTPUT_FILE="${results_dir}/bitbucket_scan_${clean_name}_${timestamp}.json"
    fi
}
...
main() {
    ...
    if [ "$DEBUG" = true ]; then
        mkdir -p "$LOG_DIR"
        local clean_name=$(echo "$REPO_NAME" | tr '/' '_')
        LOG_FILE="$LOG_DIR/bitbucket_scan_${clean_name}_$(date +%Y%m%d_%H%M%S).log"
    ...
    if [ "$findings_count" -eq 0 ]; then
        # No secrets found - clean up the result directory
        local result_dir=$(dirname "$OUTPUT_FILE")
        log_info "No secrets found - cleaning up empty result directory: $result_dir"
        rm -rf "$result_dir"
    else

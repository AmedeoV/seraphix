...
# Phrase Access Token Analyzer
# Analyzes leaked Phrase Access Token secrets and assesses their security risk
# 
# Phrase provides localization management. Tokens can access translation projects.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/PhraseAccessToken"
...
# Verify Phrase Access Token secret
...
calculate_risk_score() {
    ...
    case "$status" in
        "ACTIVE")
            echo 80  # CRITICAL - Verified active secret
            ;;
        "UNKNOWN")
            echo 80  # CRITICAL - Detected secret (assume compromised)
            ;;
        *)
...
analyze_organization() {
    ...
    log_info "Analyzing Phrase Access Token secrets for organization: $org_name"
    ...
    # Extract Phrase Access Token secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "PhraseAccessToken"
            sed -n '/\"DetectorName\": \"PhraseAccessToken\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No Phrase Access Token secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"PhraseAccessToken\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets Phrase Access Token secret(s)"
    ...
    {
      "secret_id": "phraseaccesstoken_${secret_id}",
    ...
    cat > "$output_file" <<JSONSTART
{
  ...
  "detector_type": "PhraseAccessToken",
...
# Analyze all organizations with Phrase Access Token secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with Phrase Access Token secrets..."
    
    # Find all unique organizations with Phrase Access Token secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"PhraseAccessToken\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with Phrase Access Token secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with Phrase Access Token secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "Phrase Access Token Analysis Complete!"
...
main() {
    ...
    log_info "Starting Phrase Access Token analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "Phrase Access Token analysis completed successfully!"
        else
            log_error "Phrase Access Token analysis failed with exit code: $exit_code"
        fi
    fi

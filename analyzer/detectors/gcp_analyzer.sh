...
# Google Cloud Platform Analyzer
# Analyzes leaked Google Cloud Platform secrets and assesses their security risk
# 
# GCP credentials provide access to Google Cloud services including compute, storage, and databases.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/GCP"
...
# Verify Google Cloud Platform secret
...
verify_secret() {
    ...
    echo "$service_account_json" > "$temp_json"
    
    # Test with gcloud if available
    if command -v gcloud >/dev/null 2>&1; then
        local result
        result=$(gcloud auth activate-service-account --key-file="$temp_json" 2>&1 || true)
        
        if echo "$result" | grep -q "Activated"; then
            gcloud auth revoke >/dev/null 2>&1 || true
            rm -f "$temp_json"
            echo "ACTIVE"
        elif echo "$result" | grep -qi "invalid\|disabled\|deleted"; then
            rm -f "$temp_json"
            echo "REVOKED"
        else
            rm -f "$temp_json"
        ...
        if echo "$service_account_json" | jq -e '.private_key and .client_email' >/dev/null 2>&1; then
            echo "UNKNOWN"
        else
            echo "UNKNOWN"
        fi
...
calculate_risk_score() {
    ...
    case "$status" in
        "ACTIVE")
            echo 98  # CRITICAL - Verified active secret
            ;;
        "UNKNOWN")
            echo 98  # CRITICAL - Detected secret (assume compromised)
            ;;
        *)
...
analyze_organization() {
    ...
    log_info "Analyzing Google Cloud Platform secrets for organization: $org_name"
    ...
    # Extract Google Cloud Platform secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "GCP"
            sed -n '/\"DetectorName\": \"GCP\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No Google Cloud Platform secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"GCP\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets Google Cloud Platform secret(s)"
...
# Analyze all organizations with Google Cloud Platform secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with Google Cloud Platform secrets..."
    
    # Find all unique organizations with Google Cloud Platform secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"GCP\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with Google Cloud Platform secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with Google Cloud Platform secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "Google Cloud Platform Analysis Complete!"
...
main() {
    ...
    log_info "Starting Google Cloud Platform analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "Google Cloud Platform analysis completed successfully!"
        else
            log_error "Google Cloud Platform analysis failed with exit code: $exit_code"
        fi
    fi

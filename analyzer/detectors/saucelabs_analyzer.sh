...
# Sauce Labs Analyzer
# Analyzes leaked Sauce Labs secrets and assesses their security risk
# 
# Sauce Labs provides browser and mobile testing. Credentials allow test execution.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/SauceLabs"
...
analyze_organization() {
    ...
    log_info "Analyzing Sauce Labs secrets for organization: $org_name"
    ...
    # Extract Sauce Labs secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "SauceLabs"
            sed -n '/\"DetectorName\": \"SauceLabs\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No Sauce Labs secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"SauceLabs\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets Sauce Labs secret(s)"
    ...
    {
      "secret_id": "saucelabs_${secret_id}",
...
# Analyze all organizations with Sauce Labs secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with Sauce Labs secrets..."
    
    # Find all unique organizations with Sauce Labs secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"SauceLabs\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with Sauce Labs secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with Sauce Labs secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "Sauce Labs Analysis Complete!"
...
main() {
    ...
    log_info "Starting Sauce Labs analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "Sauce Labs analysis completed successfully!"
        else
            log_error "Sauce Labs analysis failed with exit code: $exit_code"
        fi
    fi

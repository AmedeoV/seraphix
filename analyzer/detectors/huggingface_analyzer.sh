...
# Analyzes leaked Hugging Face secrets and assesses their security risk
# 
# Hugging Face tokens provide access to AI models, datasets, and inference APIs.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/HuggingFace"
...
analyze_organization() {
    ...
    log_info "Analyzing Hugging Face secrets for organization: $org_name"
    ...
    # Extract Hugging Face secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "HuggingFace"
            sed -n '/\"DetectorName\": \"HuggingFace\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No Hugging Face secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"HuggingFace\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets Hugging Face secret(s)"
    ...
    {
      "secret_id": "huggingface_${secret_id}",
...
# Analyze all organizations with Hugging Face secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with Hugging Face secrets..."
    
    # Find all unique organizations with Hugging Face secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"HuggingFace\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with Hugging Face secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with Hugging Face secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "Hugging Face Analysis Complete!"
...
main() {
    ...
    log_info "Starting Hugging Face analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "Hugging Face analysis completed successfully!"
        else
            log_error "Hugging Face analysis failed with exit code: $exit_code"
        fi
    fi

...
# MagicBell API Analyzer
# Analyzes leaked MagicBell API secrets and assesses their security risk
# 
# MagicBell provides notification infrastructure. Keys can send and manage notifications.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/MagicBell"
...
analyze_organization() {
    ...
    log_info "Analyzing MagicBell API secrets for organization: $org_name"
    ...
    # Extract MagicBell API secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "MagicBell"
            sed -n '/\"DetectorName\": \"MagicBell\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No MagicBell API secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"MagicBell\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets MagicBell API secret(s)"
    ...
    {
      "secret_id": "magicbell_${secret_id}",
...
# Analyze all organizations with MagicBell API secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with MagicBell API secrets..."
    
    # Find all unique organizations with MagicBell API secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"MagicBell\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with MagicBell API secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with MagicBell API secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "MagicBell API Analysis Complete!"
...
main() {
    ...
    log_info "Starting MagicBell API analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "MagicBell API analysis completed successfully!"
        else
            log_error "MagicBell API analysis failed with exit code: $exit_code"
        fi
    fi

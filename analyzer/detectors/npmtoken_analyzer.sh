...
# NPM Token Analyzer
# Analyzes leaked NPM Token secrets and assesses their security risk
# 
# NPM tokens provide package registry access for publishing and management.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/NpmToken"
...
# Verify NPM Token secret
# Returns: "UNKNOWN" (detected but not verified without live testing)
verify_secret() {
    local token="$1"
    
    # Test npm token with whoami endpoint
    local response
    response=$(curl -s -w "\n%{http_code}" -H "Authorization: Bearer $token" \
        "https://registry.npmjs.org/-/whoami" --max-time 10 2>/dev/null || echo -e "\n000")
...
analyze_organization() {
    ...
    log_info "Analyzing NPM Token secrets for organization: $org_name"
    ...
    # Extract NPM Token secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "NpmToken"
            sed -n '/\"DetectorName\": \"NpmToken\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No NPM Token secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"NpmToken\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets NPM Token secret(s)"
...
# Analyze all organizations with NPM Token secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with NPM Token secrets..."
    
    # Find all unique organizations with NPM Token secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"NpmToken\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with NPM Token secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with NPM Token secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "NPM Token Analysis Complete!"
...
main() {
    ...
    log_info "Starting NPM Token analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "NPM Token analysis completed successfully!"
        else
            log_error "NPM Token analysis failed with exit code: $exit_code"
        fi
    fi

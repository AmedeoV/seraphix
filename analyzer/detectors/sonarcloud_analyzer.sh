...
# SonarCloud Analyzer
# Analyzes leaked SonarCloud secrets and assesses their security risk
# 
# SonarCloud tokens provide code quality and security analysis access.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/SonarCloud"
...
analyze_organization() {
    ...
    log_info "Analyzing SonarCloud secrets for organization: $org_name"
    ...
    # Extract SonarCloud secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "SonarCloud"
            sed -n '/\"DetectorName\": \"SonarCloud\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No SonarCloud secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"SonarCloud\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets SonarCloud secret(s)"
    ...
    {
      "secret_id": "sonarcloud_${secret_id}",
...
# Analyze all organizations with SonarCloud secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with SonarCloud secrets..."
    
    # Find all unique organizations with SonarCloud secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"SonarCloud\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with SonarCloud secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with SonarCloud secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "SonarCloud Analysis Complete!"
...
main() {
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            ...
            log_error "SonarCloud analysis failed with exit code: $exit_code"
        fi
    fi

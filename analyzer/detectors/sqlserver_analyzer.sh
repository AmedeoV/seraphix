...
# SQL Server Analyzer
# Analyzes leaked SQL Server secrets and assesses their security risk
# 
# SQL Server connection strings provide database access.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/SQLServer"
...
verify_secret() {
    ...
    if command -v sqlcmd >/dev/null 2>&1; then
        local result
        result=$(timeout 10 sqlcmd -S "$connection_string" -Q "SELECT 1" 2>&1 || true)
        
        if echo "$result" | grep -q "1"; then
            echo "ACTIVE"
        elif echo "$result" | grep -qi "login failed\|authentication"; then
            echo "REVOKED"
        else
            echo "UNKNOWN"
        fi
...
analyze_organization() {
    ...
    log_info "Analyzing SQL Server secrets for organization: $org_name"
    ...
    # Extract SQL Server secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "SQLServer"
            sed -n '/\"DetectorName\": \"SQLServer\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No SQL Server secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"SQLServer\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets SQL Server secret(s)"
    ...
    {
      "secret_id": "sqlserver_${secret_id}",
...
# Analyze all organizations with SQL Server secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with SQL Server secrets..."
    
    # Find all unique organizations with SQL Server secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"SQLServer\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with SQL Server secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with SQL Server secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "SQL Server Analysis Complete!"
...
main() {
    ...
    log_info "Starting SQL Server analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "SQL Server analysis completed successfully!"
        else
            log_error "SQL Server analysis failed with exit code: $exit_code"
        fi
    fi

...
# Slack Webhook Analyzer
# Analyzes leaked Slack Webhook secrets and assesses their security risk
# 
# Slack webhooks allow posting messages to Slack channels.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/SlackWebhook"
...
verify_secret() {
    ...
    # Test with minimal payload (validation only)
    local response
    response=$(curl -s -w "\n%{http_code}" -X POST "$webhook_url" \
        -H "Content-Type: application/json" \
        -d '{"text":"verification_test"}' \
    ...
    if [ "$http_code" = "200" ] || echo "$body" | grep -q "ok"; then
        echo "ACTIVE"
    elif [ "$http_code" = "404" ] || echo "$body" | grep -q "invalid"; then
        echo "REVOKED"
...
analyze_organization() {
    ...
    log_info "Analyzing Slack Webhook secrets for organization: $org_name"
    ...
    # Extract Slack Webhook secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "SlackWebhook"
            sed -n '/\"DetectorName\": \"SlackWebhook\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No Slack Webhook secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"SlackWebhook\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets Slack Webhook secret(s)"
    ...
    {
      "secret_id": "slackwebhook_${secret_id}",
...
# Analyze all organizations with Slack Webhook secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with Slack Webhook secrets..."
    
    # Find all unique organizations with Slack Webhook secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"SlackWebhook\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with Slack Webhook secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with Slack Webhook secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "Slack Webhook Analysis Complete!"
...
main() {
    ...
    log_info "Starting Slack Webhook analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "Slack Webhook analysis completed successfully!"
        else
            log_error "Slack Webhook analysis failed with exit code: $exit_code"
        fi
    fi

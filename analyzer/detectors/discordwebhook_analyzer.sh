...
# Discord Webhook Analyzer
# Analyzes leaked Discord Webhook secrets and assesses their security risk
# 
# Discord webhooks allow sending messages to Discord channels. Active webhooks can be used to spam channels, send phishing links, or exfiltrate data.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/DiscordWebhook"
...
# Verify Discord Webhook secret
# Returns: "UNKNOWN" (detected but not verified without live testing)
verify_secret() {
    local webhook_url="$1"
    
    # Test by sending a GET request (doesn't post anything)
    local response
    response=$(curl -s -w "\n%{http_code}" -X GET "$webhook_url" --max-time 10 2>/dev/null || echo -e "\n000")
    
    local http_code=$(echo "$response" | tail -n1)
    local body=$(echo "$response" | head -n-1)
    
    if [ "$http_code" = "200" ] || echo "$body" | grep -q "application_id"; then
        echo "ACTIVE"
    elif [ "$http_code" = "404" ] || echo "$body" | grep -q "Unknown Webhook"; then
        echo "REVOKED"
...
calculate_risk_score() {
    ...
    case "$status" in
        "ACTIVE")
            echo 90  # CRITICAL - Verified active secret
            ;;
        "UNKNOWN")
            echo 90  # CRITICAL - Detected secret (assume compromised)
            ;;
        *)
...
analyze_organization() {
    ...
    log_info "Analyzing Discord Webhook secrets for organization: $org_name"
    ...
    # Extract Discord Webhook secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "DiscordWebhook"
            sed -n '/\"DetectorName\": \"DiscordWebhook\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No Discord Webhook secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"DiscordWebhook\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets Discord Webhook secret(s)"
    ...
    {
      "secret_id": "discordwebhook_${secret_id}",
...
# Analyze all organizations with Discord Webhook secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with Discord Webhook secrets..."
    
    # Find all unique organizations with Discord Webhook secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"DiscordWebhook\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with Discord Webhook secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with Discord Webhook secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "Discord Webhook Analysis Complete!"
...
main() {
    ...
    log_info "Starting Discord Webhook analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "Discord Webhook analysis completed successfully!"
        else
            log_error "Discord Webhook analysis failed with exit code: $exit_code"
        fi
    fi

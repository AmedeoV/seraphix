...
# Vercel Analyzer
# Analyzes leaked Vercel secrets and assesses their security risk
# 
# Vercel tokens provide deployment and project management access.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/Vercel"
...
analyze_organization() {
    ...
    log_info "Analyzing Vercel secrets for organization: $org_name"
    ...
    # Extract Vercel secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "Vercel"
            sed -n '/\"DetectorName\": \"Vercel\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No Vercel secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"Vercel\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets Vercel secret(s)"
...
# Analyze all organizations with Vercel secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with Vercel secrets..."
    
    # Find all unique organizations with Vercel secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"Vercel\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with Vercel secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with Vercel secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "Vercel Analysis Complete!"
...
main() {
    ...
    log_info "Starting Vercel analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "Vercel analysis completed successfully!"
        else
            log_error "Vercel analysis failed with exit code: $exit_code"
        fi
    fi

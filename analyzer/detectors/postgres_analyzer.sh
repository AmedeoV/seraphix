...
# PostgreSQL Analyzer
# Analyzes leaked PostgreSQL secrets and assesses their security risk
# 
# PostgreSQL connection strings provide database access with potential full permissions.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/Postgres"
...
verify_secret() {
    ...
    if command -v psql >/dev/null 2>&1; then
        local result
        result=$(timeout 10 psql "$connection_string" -c "SELECT 1" 2>&1 || true)
        
        if echo "$result" | grep -q "1 row"; then
            echo "ACTIVE"
        elif echo "$result" | grep -qi "authentication.*failed\|password.*failed\|no password supplied"; then
            echo "REVOKED"
        elif echo "$result" | grep -qi "does not exist\|could not connect"; then
            echo "REVOKED"
        else
            echo "UNKNOWN"
        fi
    else
        # Fallback: Parse connection string for validation
        if echo "$connection_string" | grep -qE "postgres(ql)?://"; then
            echo "UNKNOWN"  # Valid format but cannot verify
        else
            echo "UNKNOWN"
        fi
    fi
...
analyze_organization() {
    ...
    log_info "Analyzing PostgreSQL secrets for organization: $org_name"
    ...
    # Extract PostgreSQL secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "Postgres"
            sed -n '/\"DetectorName\": \"Postgres\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No PostgreSQL secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"Postgres\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets PostgreSQL secret(s)"
...
# Analyze all organizations with PostgreSQL secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with PostgreSQL secrets..."
    
    # Find all unique organizations with PostgreSQL secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"Postgres\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with PostgreSQL secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with PostgreSQL secrets"
    echo "" >&2
...
main() {
    ...
    log_info "Starting PostgreSQL analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "PostgreSQL analysis completed successfully!"
        else
            log_error "PostgreSQL analysis failed with exit code: $exit_code"
        fi
    fi

...
# Netlify Analyzer
# Analyzes leaked Netlify secrets and assesses their security risk
# 
# Netlify tokens provide site deployment and management access.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/Netlify"
...
analyze_organization() {
    ...
    log_info "Analyzing Netlify secrets for organization: $org_name"
    ...
    # Extract Netlify secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "Netlify"
            sed -n '/\"DetectorName\": \"Netlify\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No Netlify secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"Netlify\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets Netlify secret(s)"
...
# Analyze all organizations with Netlify secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with Netlify secrets..."
    
    # Find all unique organizations with Netlify secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"Netlify\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with Netlify secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with Netlify secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "Netlify Analysis Complete!"
...
main() {
    ...
    log_info "Starting Netlify analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "Netlify analysis completed successfully!"
        else
            log_error "Netlify analysis failed with exit code: $exit_code"
        fi
    fi

...
# Twitter/X Consumer Key Analyzer
# Analyzes leaked Twitter/X Consumer Key secrets and assesses their security risk
# 
# Twitter consumer keys provide API access for posting and data retrieval.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/TwitterConsumerkey"
...
# Verify Twitter/X Consumer Key secret
...
analyze_organization() {
    ...
    log_info "Analyzing Twitter/X Consumer Key secrets for organization: $org_name"
    ...
    # Extract Twitter/X Consumer Key secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "TwitterConsumerkey"
            sed -n '/\"DetectorName\": \"TwitterConsumerkey\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No Twitter/X Consumer Key secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"TwitterConsumerkey\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets Twitter/X Consumer Key secret(s)"
    ...
    {
      "secret_id": "twitterconsumerkey_${secret_id}",
    ...
    cat > "$output_file" <<JSONSTART
{
  ...
  "detector_type": "TwitterConsumerkey",
...
# Analyze all organizations with Twitter/X Consumer Key secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with Twitter/X Consumer Key secrets..."
    
    # Find all unique organizations with Twitter/X Consumer Key secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"TwitterConsumerkey\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with Twitter/X Consumer Key secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with Twitter/X Consumer Key secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "Twitter/X Consumer Key Analysis Complete!"
...
main() {
    ...
    log_info "Starting Twitter/X Consumer Key analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "Twitter/X Consumer Key analysis completed successfully!"
        else
            log_error "Twitter/X Consumer Key analysis failed with exit code: $exit_code"
        fi
    fi

...
# Smartsheet API Analyzer
# Analyzes leaked Smartsheet API secrets and assesses their security risk
# 
# Smartsheet API tokens provide access to sheets, workspaces, and data.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/Smartsheets"
...
analyze_organization() {
    ...
    log_info "Analyzing Smartsheet API secrets for organization: $org_name"
    ...
    # Extract Smartsheet API secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "Smartsheets"
            sed -n '/\"DetectorName\": \"Smartsheets\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No Smartsheet API secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"Smartsheets\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets Smartsheet API secret(s)"
    ...
    {
      "secret_id": "smartsheets_${secret_id}",
...
# Analyze all organizations with Smartsheet API secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with Smartsheet API secrets..."
    
    # Find all unique organizations with Smartsheet API secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"Smartsheets\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with Smartsheet API secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with Smartsheet API secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "Smartsheet API Analysis Complete!"
...
main() {
    ...
    log_info "Starting Smartsheet API analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "Smartsheet API analysis completed successfully!"
        else
            log_error "Smartsheet API analysis failed with exit code: $exit_code"
        fi
    fi

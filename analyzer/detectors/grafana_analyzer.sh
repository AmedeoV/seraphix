...
# Grafana API Analyzer
# Analyzes leaked Grafana API secrets and assesses their security risk
# 
# Grafana API keys provide access to dashboards, data sources, and monitoring data.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/Grafana"
...
# Verify Grafana API secret
# Returns: "UNKNOWN" (detected but not verified without live testing)
verify_secret() {
    local api_key="$1"
    
    # Note: Grafana requires instance URL, so we can only validate format
    if [ ${#api_key} -ge 20 ]; then
        echo "UNKNOWN"  # Valid format but need instance URL to verify
    else
        echo "UNKNOWN"
    fi
}
...
analyze_organization() {
    ...
    log_info "Analyzing Grafana API secrets for organization: $org_name"
    ...
    # Extract Grafana API secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "Grafana"
            sed -n '/\"DetectorName\": \"Grafana\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No Grafana API secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"Grafana\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets Grafana API secret(s)"
...
# Analyze all organizations with Grafana API secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with Grafana API secrets..."
    
    # Find all unique organizations with Grafana API secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"Grafana\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with Grafana API secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with Grafana API secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "Grafana API Analysis Complete!"
...
main() {
    ...
    log_info "Starting Grafana API analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "Grafana API analysis completed successfully!"
        else
            log_error "Grafana API analysis failed with exit code: $exit_code"
        fi
    fi

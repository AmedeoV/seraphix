...
# Etherscan API Analyzer
# Analyzes leaked Etherscan API secrets and assesses their security risk
# 
# Etherscan API provides Ethereum blockchain data access. Keys allow querying transactions, addresses, and smart contracts.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/Etherscan"
...
verify_secret() {
    ...
    local response
    response=$(curl -s -w "\n%{http_code}" \
        "https://api.etherscan.io/api?module=account&action=balance&address=0x0000000000000000000000000000000000000000&tag=latest&apikey=$api_key" \
    ...
    if [ "$http_code" = "200" ] && echo "$body" | grep -q '"status":"1"'; then
        echo "ACTIVE"
    elif echo "$body" | grep -qi "invalid.*api.*key\|invalid key"; then
        echo "REVOKED"
    elif echo "$body" | grep -qi "rate limit"; then
        echo "RATE_LIMITED"
    else
        echo "UNKNOWN"
    fi
...
calculate_risk_score() {
    ...
    case "$status" in
        "ACTIVE")
            echo 75  # CRITICAL - Verified active secret
            ;;
        "UNKNOWN")
            echo 75  # CRITICAL - Detected secret (assume compromised)
            ;;
        *)
...
analyze_organization() {
    ...
    log_info "Analyzing Etherscan API secrets for organization: $org_name"
    ...
    # Extract Etherscan API secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "Etherscan"
            sed -n '/\"DetectorName\": \"Etherscan\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No Etherscan API secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"Etherscan\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets Etherscan API secret(s)"
    ...
    {
      "secret_id": "etherscan_${secret_id}",
...
# Analyze all organizations with Etherscan API secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with Etherscan API secrets..."
    
    # Find all unique organizations with Etherscan API secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"Etherscan\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with Etherscan API secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with Etherscan API secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "Etherscan API Analysis Complete!"
...
main() {
    ...
    log_info "Starting Etherscan API analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "Etherscan API analysis completed successfully!"
        else
            log_error "Etherscan API analysis failed with exit code: $exit_code"
        fi
    fi

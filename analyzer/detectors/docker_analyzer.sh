...
# Analyzes leaked Docker Registry secrets and assesses their security risk
# 
# Docker registry credentials provide access to private container images and repositories.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/Docker"
...
verify_secret() {
    ...
    # Parse username:password or token
    if echo "$credentials" | grep -q ":"; then
        ...
        if [ "$http_code" = "200" ] && echo "$body" | grep -q "token"; then
            echo "ACTIVE"
        elif [ "$http_code" = "401" ] || echo "$body" | grep -qi "incorrect"; then
            echo "REVOKED"
        else
            echo "UNKNOWN"
        fi
...
analyze_organization() {
    ...
    log_info "Analyzing Docker Registry secrets for organization: $org_name"
    ...
    # Extract Docker Registry secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "Docker"
            sed -n '/\"DetectorName\": \"Docker\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No Docker Registry secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"Docker\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets Docker Registry secret(s)"
...
# Analyze all organizations with Docker Registry secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with Docker Registry secrets..."
    
    # Find all unique organizations with Docker Registry secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"Docker\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with Docker Registry secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with Docker Registry secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "Docker Registry Analysis Complete!"
...
main() {
    ...
    log_info "Starting Docker Registry analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "Docker Registry analysis completed successfully!"
        else
            log_error "Docker Registry analysis failed with exit code: $exit_code"
        fi
    fi

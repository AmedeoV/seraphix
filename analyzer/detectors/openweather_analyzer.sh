...
# OpenWeather API Analyzer
# Analyzes leaked OpenWeather API secrets and assesses their security risk
# 
# OpenWeather API keys provide weather data access.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/OpenWeather"
...
verify_secret() {
    ...
    if [ "$http_code" = "200" ] && echo "$body" | grep -q "weather"; then
        echo "ACTIVE"
    elif [ "$http_code" = "401" ] || echo "$body" | grep -qi "invalid.*api.*key"; then
        echo "REVOKED"
...
calculate_risk_score() {
    ...
    case "$status" in
        "ACTIVE")
            echo 70  # CRITICAL - Verified active secret
            ;;
        "UNKNOWN")
            echo 70  # CRITICAL - Detected secret (assume compromised)
            ;;
        *)
...
analyze_organization() {
    ...
    log_info "Analyzing OpenWeather API secrets for organization: $org_name"
    ...
    # Extract OpenWeather API secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "OpenWeather"
            sed -n '/\"DetectorName\": \"OpenWeather\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No OpenWeather API secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"OpenWeather\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets OpenWeather API secret(s)"
    ...
    {
      "secret_id": "openweather_${secret_id}",
...
# Analyze all organizations with OpenWeather API secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with OpenWeather API secrets..."
    
    # Find all unique organizations with OpenWeather API secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"OpenWeather\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with OpenWeather API secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with OpenWeather API secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "OpenWeather API Analysis Complete!"
...
main() {
    ...
    log_info "Starting OpenWeather API analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "OpenWeather API analysis completed successfully!"
        else
            log_error "OpenWeather API analysis failed with exit code: $exit_code"
        fi
    fi

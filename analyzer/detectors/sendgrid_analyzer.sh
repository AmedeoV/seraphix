...
# SendGrid API Analyzer
# Analyzes leaked SendGrid API secrets and assesses their security risk
# 
# SendGrid provides email delivery. Keys can send emails and access account data.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/SendGrid"
...
verify_secret() {
    ...
    if [ "$http_code" = "200" ] && echo "$body" | grep -q "scopes"; then
        echo "ACTIVE"
...
analyze_organization() {
    ...
    log_info "Analyzing SendGrid API secrets for organization: $org_name"
    ...
    # Extract SendGrid API secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "SendGrid"
            sed -n '/\"DetectorName\": \"SendGrid\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No SendGrid API secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"SendGrid\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets SendGrid API secret(s)"
...
# Analyze all organizations with SendGrid API secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with SendGrid API secrets..."
    
    # Find all unique organizations with SendGrid API secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"SendGrid\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with SendGrid API secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with SendGrid API secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "SendGrid API Analysis Complete!"
...
main() {
    ...
    log_info "Starting SendGrid API analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "SendGrid API analysis completed successfully!"
        else
            log_error "SendGrid API analysis failed with exit code: $exit_code"
        fi
    fi

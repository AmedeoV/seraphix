...
# MongoDB Analyzer
# Analyzes leaked MongoDB secrets and assesses their security risk
# 
# MongoDB connection strings provide database access with potential read/write permissions.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/MongoDB"
...
verify_secret() {
    ...
    # Use mongosh or mongo client to test connection
    if command -v mongosh >/dev/null 2>&1; then
        local result
        result=$(timeout 10 mongosh "$connection_string" --quiet --eval "db.adminCommand({ping: 1})" 2>&1 || true)
        
        if echo "$result" | grep -q "ok.*1"; then
            echo "ACTIVE"
        elif echo "$result" | grep -qi "authentication.*failed\|unauthorized\|auth.*failed"; then
            echo "REVOKED"
        else
            echo "UNKNOWN"
        fi
        ...
        if echo "$connection_string" | grep -qE "mongodb(\+srv)?://[^@]+@"; then
...
analyze_organization() {
    ...
    log_info "Analyzing MongoDB secrets for organization: $org_name"
    ...
    # Extract MongoDB secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "MongoDB"
            sed -n '/\"DetectorName\": \"MongoDB\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No MongoDB secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"MongoDB\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets MongoDB secret(s)"
...
# Analyze all organizations with MongoDB secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with MongoDB secrets..."
    
    # Find all unique organizations with MongoDB secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"MongoDB\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with MongoDB secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with MongoDB secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "MongoDB Analysis Complete!"
...
main() {
    ...
    log_info "Starting MongoDB analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "MongoDB analysis completed successfully!"
        else
            log_error "MongoDB analysis failed with exit code: $exit_code"
        fi
    fi

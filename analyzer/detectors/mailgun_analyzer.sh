...
# Mailgun API Analyzer
# Analyzes leaked Mailgun API secrets and assesses their security risk
# 
# Mailgun provides email delivery services. Keys can send emails and access mailing lists.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/Mailgun"
...
# Verify Mailgun API secret
# Returns: "UNKNOWN" (detected but not verified without live testing)
verify_secret() {
    local api_key="$1"
    
    # Test Mailgun API key with domain list
    local response
    response=$(curl -s -w "\n%{http_code}" -u "api:$api_key" \
    ...
    if [ "$http_code" = "200" ]; then
        echo "ACTIVE"
    elif [ "$http_code" = "401" ] || echo "$body" | grep -qi "forbidden\|unauthorized"; then
        echo "REVOKED"
    else
        echo "UNKNOWN"
    fi
...
analyze_organization() {
    ...
    log_info "Analyzing Mailgun API secrets for organization: $org_name"
    ...
    # Extract Mailgun API secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "Mailgun"
            sed -n '/\"DetectorName\": \"Mailgun\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No Mailgun API secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"Mailgun\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets Mailgun API secret(s)"
...
# Analyze all organizations with Mailgun API secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with Mailgun API secrets..."
    
    # Find all unique organizations with Mailgun API secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"Mailgun\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with Mailgun API secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with Mailgun API secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "Mailgun API Analysis Complete!"
...
main() {
    ...
    log_info "Starting Mailgun API analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "Mailgun API analysis completed successfully!"
        else
            log_error "Mailgun API analysis failed with exit code: $exit_code"
        fi
    fi

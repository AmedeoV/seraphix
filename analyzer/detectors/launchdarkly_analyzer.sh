...
# LaunchDarkly Analyzer
# Analyzes leaked LaunchDarkly secrets and assesses their security risk
# 
# LaunchDarkly provides feature flag management. Keys can control application features.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/LaunchDarkly"
...
analyze_organization() {
    ...
    log_info "Analyzing LaunchDarkly secrets for organization: $org_name"
    ...
    # Extract LaunchDarkly secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "LaunchDarkly"
            sed -n '/\"DetectorName\": \"LaunchDarkly\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No LaunchDarkly secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"LaunchDarkly\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets LaunchDarkly secret(s)"
    ...
    {
      "secret_id": "launchdarkly_${secret_id}",
...
# Analyze all organizations with LaunchDarkly secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with LaunchDarkly secrets..."
    
    # Find all unique organizations with LaunchDarkly secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"LaunchDarkly\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with LaunchDarkly secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with LaunchDarkly secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "LaunchDarkly Analysis Complete!"
...
main() {
    ...
    log_info "Starting LaunchDarkly analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "LaunchDarkly analysis completed successfully!"
        else
            log_error "LaunchDarkly analysis failed with exit code: $exit_code"
        fi
    fi

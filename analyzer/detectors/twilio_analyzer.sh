...
# Twilio API Analyzer
# Analyzes leaked Twilio API secrets and assesses their security risk
# 
# Twilio provides SMS, voice, and video communication. Keys can send messages and make calls.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/Twilio"
...
# Verify Twilio API secret
# Returns: "UNKNOWN" (detected but not verified without live testing)
verify_secret() {
    local credentials="$1"
    
    # Parse Account SID and Auth Token (format: ACCOUNT_SID:AUTH_TOKEN)
    if echo "$credentials" | grep -q ":"; then
        local account_sid=$(echo "$credentials" | cut -d: -f1)
        local auth_token=$(echo "$credentials" | cut -d: -f2-)
        
        # Test Twilio credentials
        local response
        response=$(curl -s -w "\n%{http_code}" -u "$account_sid:$auth_token" \
        ...
        if [ "$http_code" = "200" ] && echo "$body" | grep -q "sid"; then
            echo "ACTIVE"
...
analyze_organization() {
    ...
    log_info "Analyzing Twilio API secrets for organization: $org_name"
    ...
    # Extract Twilio API secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "Twilio"
            sed -n '/\"DetectorName\": \"Twilio\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No Twilio API secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"Twilio\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets Twilio API secret(s)"
...
# Analyze all organizations with Twilio API secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with Twilio API secrets..."
    
    # Find all unique organizations with Twilio API secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"Twilio\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with Twilio API secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with Twilio API secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "Twilio API Analysis Complete!"
...
main() {
    ...
    log_info "Starting Twilio API analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "Twilio API analysis completed successfully!"
        else
            log_error "Twilio API analysis failed with exit code: $exit_code"
        fi
    fi

...
# GitHub OAuth2 Analyzer
# Analyzes leaked GitHub OAuth2 secrets and assesses their security risk
# 
# GitHub OAuth2 tokens provide repository and user access with various scopes.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/GitHubOauth2"
...
# Verify GitHub OAuth2 secret
# Returns: "UNKNOWN" (detected but not verified without live testing)
verify_secret() {
    local token="$1"
    
    # Test GitHub token with user endpoint
    local response
    response=$(curl -s -w "\n%{http_code}" -H "Authorization: token $token" \
        "https://api.github.com/user" --max-time 10 2>/dev/null || echo -e "\n000")
    ...
    if [ "$http_code" = "200" ] && echo "$body" | grep -q "login"; then
        echo "ACTIVE"
    elif [ "$http_code" = "401" ] || echo "$body" | grep -q "Bad credentials"; then
        echo "REVOKED"
    elif [ "$http_code" = "403" ] && echo "$body" | grep -q "rate limit"; then
        echo "RATE_LIMITED"
    else
        echo "UNKNOWN"
    fi
...
calculate_risk_score() {
    ...
    case "$status" in
        "ACTIVE")
            echo 95  # CRITICAL - Verified active secret
            ;;
        "UNKNOWN")
            echo 95  # CRITICAL - Detected secret (assume compromised)
            ;;
        *)
...
analyze_organization() {
    ...
    log_info "Analyzing GitHub OAuth2 secrets for organization: $org_name"
    ...
    # Extract GitHub OAuth2 secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "GitHubOauth2"
            sed -n '/\"DetectorName\": \"GitHubOauth2\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No GitHub OAuth2 secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"GitHubOauth2\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets GitHub OAuth2 secret(s)"
    ...
    {
      "secret_id": "githuboauth2_${secret_id}",
...
# Analyze all organizations with GitHub OAuth2 secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with GitHub OAuth2 secrets..."
    
    # Find all unique organizations with GitHub OAuth2 secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"GitHubOauth2\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with GitHub OAuth2 secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with GitHub OAuth2 secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "GitHub OAuth2 Analysis Complete!"
...
main() {
    ...
    log_info "Starting GitHub OAuth2 analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "GitHub OAuth2 analysis completed successfully!"
        else
            log_error "GitHub OAuth2 analysis failed with exit code: $exit_code"
        fi
    fi

...
# RabbitMQ Analyzer
# Analyzes leaked RabbitMQ secrets and assesses their security risk
# 
# RabbitMQ credentials provide message queue access.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/RabbitMQ"
...
get_capabilities() {
    ...
    if [ "$status" = "ACTIVE" ] || [ "$status" = "UNKNOWN" ]; then
        cat <<EOF
    "queue_access": true,
    "message_publish": true,
    "message_consume": true,
...
analyze_organization() {
    ...
    log_info "Analyzing RabbitMQ secrets for organization: $org_name"
    ...
    # Extract RabbitMQ secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "RabbitMQ"
            sed -n '/\"DetectorName\": \"RabbitMQ\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No RabbitMQ secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"RabbitMQ\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets RabbitMQ secret(s)"
...
# Analyze all organizations with RabbitMQ secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with RabbitMQ secrets..."
    
    # Find all unique organizations with RabbitMQ secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"RabbitMQ\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with RabbitMQ secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with RabbitMQ secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "RabbitMQ Analysis Complete!"
...
main() {
    ...
    log_info "Starting RabbitMQ analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "RabbitMQ analysis completed successfully!"
        else
            log_error "RabbitMQ analysis failed with exit code: $exit_code"
        fi
    fi

...
# Analyzes leaked Private Key secrets and assesses their security risk
# 
# Private cryptographic keys can provide access to various systems and sign transactions.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/PrivateKey"
...
analyze_organization() {
    ...
    log_info "Analyzing Private Key secrets for organization: $org_name"
    ...
    # Extract Private Key secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "PrivateKey"
            sed -n '/\"DetectorName\": \"PrivateKey\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No Private Key secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"PrivateKey\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets Private Key secret(s)"
    ...
    {
      "secret_id": "privatekey_${secret_id}",
...
# Analyze all organizations with Private Key secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with Private Key secrets..."
    
    # Find all unique organizations with Private Key secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"PrivateKey\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with Private Key secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with Private Key secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "Private Key Analysis Complete!"
...
main() {
    ...
    log_info "Starting Private Key analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "Private Key analysis completed successfully!"
        else
            log_error "Private Key analysis failed with exit code: $exit_code"
        fi
    fi

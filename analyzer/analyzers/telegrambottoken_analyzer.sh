...
# Analyzes leaked Telegram Bot secrets and assesses their security risk
# 
# Telegram bot tokens allow full control over a bot including message sending.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/TelegramBotToken"
...
verify_secret() {
    ...
    if [ "$http_code" = "200" ] && echo "$body" | grep -q '"ok":true'; then
        echo "ACTIVE"
    elif [ "$http_code" = "401" ] || echo "$body" | grep -qi "unauthorized\|not found"; then
        echo "REVOKED"
...
analyze_organization() {
    ...
    log_info "Analyzing Telegram Bot secrets for organization: $org_name"
    ...
    # Extract Telegram Bot secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "TelegramBotToken"
            sed -n '/\"DetectorName\": \"TelegramBotToken\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No Telegram Bot secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"TelegramBotToken\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets Telegram Bot secret(s)"
    ...
    {
      "secret_id": "telegrambottoken_${secret_id}",
    ...
    cat > "$output_file" <<JSONSTART
{
  ...
  "detector_type": "TelegramBotToken",
...
# Analyze all organizations with Telegram Bot secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with Telegram Bot secrets..."
    
    # Find all unique organizations with Telegram Bot secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"TelegramBotToken\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with Telegram Bot secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with Telegram Bot secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "Telegram Bot Analysis Complete!"
...
main() {
    ...
    log_info "Starting Telegram Bot analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "Telegram Bot analysis completed successfully!"
        else
            log_error "Telegram Bot analysis failed with exit code: $exit_code"
        fi
    fi

...
# Infura API Analyzer
# Analyzes leaked Infura API secrets and assesses their security risk
# 
# Infura provides blockchain node access. Keys allow Ethereum and IPFS API access.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/Infura"
...
# Verify Infura API secret
# Returns: "UNKNOWN" (detected but not verified without live testing)
verify_secret() {
    local api_key="$1"
    
    # Test Infura key with eth_blockNumber
    local base_url="https://mainnet.infura.io/v3/${api_key}"
    local response
    ...
    if [ "$http_code" = "200" ] && echo "$body" | grep -q "result"; then
        echo "ACTIVE"
    elif [ "$http_code" = "401" ] || [ "$http_code" = "403" ] || echo "$body" | grep -qi "invalid.*project"; then
        echo "REVOKED"
...
analyze_organization() {
    ...
    log_info "Analyzing Infura API secrets for organization: $org_name"
    ...
    # Extract Infura API secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "Infura"
            sed -n '/\"DetectorName\": \"Infura\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No Infura API secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"Infura\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets Infura API secret(s)"
...
# Analyze all organizations with Infura API secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with Infura API secrets..."
    
    # Find all unique organizations with Infura API secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"Infura\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with Infura API secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with Infura API secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "Infura API Analysis Complete!"
...
main() {
    ...
    log_info "Starting Infura API analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "Infura API analysis completed successfully!"
        else
            log_error "Infura API analysis failed with exit code: $exit_code"
        fi
    fi

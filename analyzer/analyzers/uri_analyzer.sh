...
# URI Credentials Analyzer
# Analyzes leaked URI Credentials secrets and assesses their security risk
# 
# URI credentials may contain embedded authentication for various services.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/URI"
...
analyze_organization() {
    ...
    log_info "Analyzing URI Credentials secrets for organization: $org_name"
    ...
    # Extract URI Credentials secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "URI"
            sed -n '/\"DetectorName\": \"URI\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No URI Credentials secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"URI\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets URI Credentials secret(s)"
...
# Analyze all organizations with URI Credentials secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with URI Credentials secrets..."
    
    # Find all unique organizations with URI Credentials secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"URI\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with URI Credentials secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with URI Credentials secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "URI Credentials Analysis Complete!"
...
main() {
    ...
    log_info "Starting URI Credentials analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "URI Credentials analysis completed successfully!"
        else
            log_error "URI Credentials analysis failed with exit code: $exit_code"
        fi
    fi

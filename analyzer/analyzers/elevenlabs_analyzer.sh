...
# ElevenLabs API Analyzer
# Analyzes leaked ElevenLabs API secrets and assesses their security risk
# 
# ElevenLabs provides AI voice synthesis. Active keys can generate voice content and access account resources.
#
...
ANALYZED_RESULTS_DIR="$ANALYZER_DIR/analyzed_results/ElevenLabs"
...
analyze_organization() {
    ...
    log_info "Analyzing ElevenLabs API secrets for organization: $org_name"
    ...
    # Extract ElevenLabs API secrets from all files
    local detector_secrets
    detector_secrets=$(echo "$secrets_files" | while read -r file; do
        if [ -f "$file" ]; then
            # Extract secrets with DetectorName = "ElevenLabs"
            sed -n '/\"DetectorName\": \"ElevenLabs\"/,/^  },\{0,1\}$/p' "$file" 2>/dev/null || true
        fi
    done)
    
    if [ -z "$detector_secrets" ]; then
        log_info "No ElevenLabs API secrets found for organization: $org_name"
        return 0
    fi
    
    # Count secrets
    local total_secrets
    total_secrets=$(echo "$detector_secrets" | grep -c '\"DetectorName\": \"ElevenLabs\"' 2>/dev/null || echo "0")
    ...
    log_info "Found $total_secrets ElevenLabs API secret(s)"
    ...
    {
      "secret_id": "elevenlabs_${secret_id}",
...
# Analyze all organizations with ElevenLabs API secrets
analyze_all_organizations() {
    local secrets_dir="$1"
    
    log_info "Scanning for organizations with ElevenLabs API secrets..."
    
    # Find all unique organizations with ElevenLabs API secrets
    local organizations
    organizations=$(find "$secrets_dir" -name "verified_secrets_*.json" -type f 2>/dev/null | while read -r file; do
        if grep -q '\"DetectorName\": \"ElevenLabs\"' "$file" 2>/dev/null; then
            basename "$file" | sed 's/verified_secrets_//' | sed 's/.json//'
        fi
    done | sort -u)
    
    if [ -z "$organizations" ]; then
        log_error "No organizations with ElevenLabs API secrets found!"
        return 1
    fi
    
    local org_count
    org_count=$(echo "$organizations" | wc -l)
    log_info "Found $org_count organization(s) with ElevenLabs API secrets"
    echo "" >&2
    ...
    echo "================================================" >&2
    log_success "ElevenLabs API Analysis Complete!"
...
main() {
    ...
    log_info "Starting ElevenLabs API analysis"
    ...
    if [ "$analyze_all" = true ]; then
        ...
        if [ $exit_code -eq 0 ]; then
            log_success "ElevenLabs API analysis completed successfully!"
        else
            log_error "ElevenLabs API analysis failed with exit code: $exit_code"
        fi
    fi
